...
if (isset ( $_GET['cstart'] ) AND $_GET['cstart'] < 1 ) $_GET['cstart'] = 1;
...
//####################################################################################################################
//             Собственные метатеги и заголовки для страниц
//####################################################################################################################
...
if( $config['allow_own_meta'] ) {
	...
	if(count($custom_metatags['simple']) AND $custom_metatags['simple'][$r_uri] ) {
...
if ($config['allow_calendar'] OR $config['allow_archives']) include_once ENGINE_DIR . '/modules/calendar.php';
